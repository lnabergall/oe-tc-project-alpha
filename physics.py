...
def potential_at(R, Q, point_func, i, r, pauli_exclusion=True):
	"""
...
def energy_difference(i, q, r1, r2, potential_func):
	potential_diff = potential_func(i, r2) - potential_func(i, r1)
	return potential_energy(potential_diff, q)
...
def compute_velocity_bound(r, interaction_field, slow_field, external_field, 
						   energy_field, mass, charge, gamma, time_unit):
	net_force = (charge * interaction_field[r]) + mass * (slow_field[r] + external_field[r])
	distance = (jnp.linalg.vector_norm(net_force) * time_unit) / (gamma * mass)
	distance += jnp.sqrt(2 * energy_field[r] / mass) * time_unit
	return distance


def energy_barrier(i, path, potential_func, coupling_constant, lower_bound):
	"""
	...
	def step_fn(i, max_energy):
		energy = coupling_constant * potential_func(i, path[i])
		return jnp.max(jnp.array((energy, max_energy)))

	max_energy = jax.lax.fori_loop(0, end + 1, step_fn, lower_bound)
	potential_func_batch = jax.vmap(potential_func, in_axis=(None, 0))
	boundary_max_energy = jnp.max(coupling_constant * potential_func_batch(i, path[(0, end), :]))
...
def calculate_total_work(total_path_work, slow_field, external_field, mass, r_start, r_end, d):
...
def calculate_end_field(slow_field, external_field, energy_field, transfer_field, i, r_start, r_end, gamma):
	extra_field = jnp.sqrt(2 * energy_field[r_start] / mass) * gamma
	return slow_field[r_start] + external_field[r_end] + extra_field + transfer_field[i]
...
def generate_full_transferred_field(I, net_field, R, bound_states, masses, coms, MOIs, pad_value):
	"""
	I
	...
	"""
	masses_by_particle = masses[bound_states[I]]
	coms_by_particle = coms[bound_states[I]]
	MOIs_by_particle = MOIs[bound_states[I]]
	torques = calculate_torque(R, coms_by_particle, net_field)
	weighted_torques = torques / MOIs_by_particle
	weighted_field = net_field / masses_by_particle

	def calculate_transfer(i):
		molecule_id = bound_states[i]
		transfers = bound_states == molecule_id
		in_fields = jnp.where(transfers, weighted_field, 0)
		in_torques = jnp.where(transfers, weighted_torques, 0)
		in_position_diffs = jnp.where(transfers, R[i] - R, 0)
		in_torque_fields = cross_1D(in_torques, in_position_diffs)
		return jnp.where(i == pad_value, 0.0, jnp.sum(in_fields + in_torque_fields, axis=0))

	transfer_field = jax.vmap(collect_elements)(I)
...
def calculate_excitation_work(r_start, r_end, energy_field):
	...
	"""
	unit_update_vec = normalize(r_end - r_start)
	excitation = energy_field[r_start]
	excitation_work = jnp.dot(excitation, unit_update_vec)
	total_excitation_work = jnp.linalg.vector_norm(energy_field[r_start])

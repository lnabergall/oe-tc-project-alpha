...
def compute_velocity_bound(r, interaction_field, brownian_field, 
						   external_field, mass, charge, gamma, time_unit):
	net_force = (charge * interaction_field[r]) + mass * (brownian_field + external_field)
...
def calculate_total_work(total_path_work, brownian_field, external_field, mass, r_start, r_end, d):
	diff_norm = jnp.linalg.norm(r_end - r_start)
	excess_force_magnitude = mass * (jnp.linalg.norm(external_field[r_end]) 
							 		 + jnp.linalg.norm(brownian_field[r_start]))

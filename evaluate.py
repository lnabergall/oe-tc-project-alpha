...
AVG_STATS = ["U_avg", "K_avg", "E_avg", "S_avg", "P_avg", "impulse_avg", 
             "external_field_avg", "brownian_field_avg"]
...
def calculate_eval_data(states):
    ...
    bs_funcs = map(jax.vmap, [sizes, count, density])
    ...
    total_stats = tuple(zip(TOTAL_STATS, total_stats[:5] + (bs_stats[1],)))
    avg_stats = tuple(zip(AVG_STATS, avg_stats))
    small_avg_stats = (avg_stats[3],) + avg_stats[5:8]
    bs_stats = tuple(zip(BS_STATS, (bs_stats[0], bs_stats[2])))
